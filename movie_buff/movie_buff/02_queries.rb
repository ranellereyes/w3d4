...
def cast_list(title)
  ...
  Actor.joins(:movies).select('actors.id, actors.name')
...
def vanity_projects
  ...
  Movie.joins(:actors).select(:id, :title, :name)
...
def most_supportive
  ...
  Actor.joins(:castings).select('actors.id, name, COUNT(castings.*) AS roles')
    .where('castings.ord > 1').group('actors.id').order('COUNT(castings.*) DESC').limit(2)
end
